...
function mirror(data) {
    let newsamedata = data.slice(0, data.length)
        newsamedata.sort((a,b)=> a-b)
    let mirroredData = []
    for (let i = newsamedata.length-2; i >= 0; i--) {
      mirroredData.push(newsamedata[i])
    }
  let finalData = newsamedata.concat(mirroredData)
   return finalData
  }